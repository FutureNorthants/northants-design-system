...
The components are purely for displaying the data, with the Frontend website responsible for querying the API and passing the results into the components in the design system. This allows the design system to focus on the look and feel of the components, and allows the frontend website to server side render the page.
...
When a taxonomy is selected and the form is submitted, it will add `&taxonomy_id=SelectedTaxonomy:1` to the query string.

### Custom Taxonomy Filters

`customTaxonomyFilters` is an array of strings that is used to decide if the taxonomy should be added to the query string in a special way, rather than as a standard taxonomy. Instead of `&taxonomy_id` it will use the vocabulary name from the category in the query string.

This allows for custom searches, such as facilities and languages, which are not taxonomies in the open referral system.
